    ...
    >
      ...
      <g>
        <path
          ...
          fill="#4285F4"
        />
        <path d="M16 6.5A7.5 7.5 0 1 0 16 21.5a7.5 7.5 0 0 0 0-15z" fill="#34A853" />
...
export default function DishCard({ recommendation, userLat, userLng }: DishCardProps) {
  ...
  if (
    ...
    const distMeters = haversineDistance(userLat, userLng, parseFloat(restaurant.lat), parseFloat(restaurant.lng))
    const isUS = true
    distanceDisplay = isUS ? `${(distMeters / 1609.34).toFixed(1)} mi` : `${distMeters.toFixed(0)} m`
  }

  const { name: dishName } = recommendation.dish
  const { name: restaurantName } = recommendation.restaurant
  // at ${restaurantName}
  const imageQuery = encodeURIComponent(`${dishName}`)
  ...
  return (
    <div className="bg-white border border-brand-border rounded-lg overflow-hidden shadow-sm hover:shadow-md transition-shadow duration-200 ease-in-out flex flex-col sm:flex-row">
      ...
      <div className="p-4 sm:p-6 flex-1">
        <h3 className="text-xl font-semibold text-brand-primary mb-1">{recommendation.dish.name}</h3>
        <p className="text-brand-text text-sm leading-relaxed mb-8">{recommendation.dish.description}</p>
        <div className="mb-2 flex flex-col gap-1">
          <div className="text-sm text-brand-text-muted font-medium">
            {restaurant.name}
            {distanceDisplay && <span className="ml-2 text-xs text-gray-500">({distanceDisplay} away)</span>}
          </div>
          {restaurant.website ? (
            <a
            ...
            >
              <span className="block text-xs text-brand-text-muted group-hover:underline">{restaurant.address}</span>
            ...
            <div>
              <div className="text-xs text-brand-text-muted">{restaurant.address}</div>
            </div>
          )}
          <a
            href={googleMapsUrl}
        ...
        {recommendation.dish.rating && (
          <div className="flex items-center mb-3">
            {[...Array(Math.floor(Number(recommendation.dish.rating)))].map((_, index) => (
              <Star
            ...
            {[...Array(5 - Math.ceil(Number(recommendation.dish.rating)))].map((_, index) => (
              <Star key={`empty-${recommendation.dish.name}-${index}`} className="h-5 w-5 text-gray-300" />
            ))}
            <span className="ml-2 text-sm text-brand-text-muted">{recommendation.dish.rating}</span>
          </div>
        )}
      </div>
    </div>
